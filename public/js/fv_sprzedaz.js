...
async function loadFaktury(page = 1) {
    ...
    const res = await fetch(`backend/mod_fv_sprzedaz.php?limit=${itemsPerPage}&offset=${offset}`);
...
async function deleteFaktura(id) {
  showConfirmModal(async () => {
    ...
    try {
      const res = await fetch('backend/mod_fv_sprzedaz.php', { method: 'POST', body: data });
...
function fakturaForm(data = {}) {
  ...
  form.onsubmit = async (e) => {
    ...
    try {
      const res = await fetch('backend/mod_fv_sprzedaz.php', { method: 'POST', body: formData });
