...
function insertPozycjeFaktury($conn, $fakturaId, $metalStr, $wagaStr) {
    ...
    for ($i = 0; $i < max(count($metale), count($wagiSurowe)); $i++) {
        ...
        $ok = pg_query_params($conn,
            "INSERT INTO pozycje_faktury_sprzedazowe (faktura_id, metal, waga) VALUES ($1, $2, $3)",
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $res1 = pg_query_params($conn,
            "INSERT INTO faktury_sprzedazowe (nr_faktury, data, firma) VALUES ($1, $2, $3) RETURNING id",
    ...
    if ($action === 'update') {
        ...
        $res = pg_query_params($conn,
            "UPDATE faktury_sprzedazowe SET nr_faktury = $1, data = $2, firma = $3 WHERE id = $4",
        ...
        pg_query_params($conn, "DELETE FROM pozycje_faktury_sprzedazowe WHERE faktura_id = $1", [$_POST['id']]);
    ...
    if ($action === 'delete') {
        $res = pg_query_params($conn, "DELETE FROM faktury_sprzedazowe WHERE id = $1", [$_POST['id']]);
